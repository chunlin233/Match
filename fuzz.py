...
@utils1.check_for_none
@utils1.check_for_equivalence
@utils1.check_empty_string
def ratio(s1, s2):
    s1, s2 = utils1.make_type_consistent(s1, s2)

    m = SequenceMatcher(None, s1, s2)
    return utils1.intr(100 * m.ratio())
...
def partial_ratio(s1, s2):
    ...
    return utils1.intr(100 * max(scores))
...
def _process_and_sort(s, force_ascii, full_process=True):
    ...
    # pull tokens
    ts = utils1.full_process(s, force_ascii=force_ascii) if full_process else s
    tokens = ts.split()
...
def _token_set(s1, s2, partial=True, force_ascii=True, full_process=True):
    ...
    p1 = utils1.full_process(s1, force_ascii=force_ascii) if full_process else s1
    p2 = utils1.full_process(s2, force_ascii=force_ascii) if full_process else s2

    if not utils1.validate_string(p1):
        return 0
    if not utils1.validate_string(p2):
        return 0
...
def QRatio(s1, s2, force_ascii=True, full_process=True):
    ...
    if full_process:
        p1 = utils1.full_process(s1, force_ascii=force_ascii)
        p2 = utils1.full_process(s2, force_ascii=force_ascii)
    else:
        p1 = s1
        p2 = s2
